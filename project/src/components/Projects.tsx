...
import { ExternalLink, Printer, LayoutGrid } from 'lucide-react';
    ...
    <section id="projects" className="relative px-6 py-24 overflow-hidden bg-white">
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="mb-16 text-center">
          ...
          <h2 className="inline-flex items-center gap-3 text-4xl font-bold text-slate-900 sm:text-5xl">
            Dự án & Bài tập
            <button
              onClick={() => setShowAll(!showAll)}
              className="group flex items-center gap-1.5 rounded-full border border-slate-300 bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500 transition-all hover:border-sky-400 hover:bg-sky-50 hover:text-sky-600"
              title={showAll ? 'Hiển thị dạng lưới' : 'Hiển thị tất cả để in'}
            >
              {showAll ? (
                <>
                  <LayoutGrid size={12} />
        ...
        {showAll ? (
          <div className="flex flex-col items-center w-full max-w-4xl mx-auto">
            {projects.map((p) => (
              <ProjectModal key={p.id} project={p} isInline={true} onClose={() => {}} />
            ))}
          </div>
        ) : (
